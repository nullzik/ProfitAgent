                                       ...
                                       const QString &unit)
{
    try {
        ...
        // Преобразуем строку единицы измерения в enum Unit
        ...
        if (unit == "Gram" || unit == "gram" || unit == "г" || unit == "Грамм") {
            ...
            unitEnum = Unit::Kilogram; // По умолчанию килограмм
        }
                                         ...
                                         double purchasePrice)
{
    try {
        ...
        // Простое преобразование в денежный объект без дополнительной логики:
        // цена в "мажорных" единицах преобразуется в minor units (например, копейки).
...
void WarehouseViewModel::reloadProducts()
{
    ...
    for (const auto &product : allProducts) {
        ...
        // Преобразуем enum Unit в строку для QML
        QString unitStr;
        switch (product.unit()) {
            case Unit::Kilogram:
