...
void WarehouseService::addProductBatch(const std::string& productId,
    ...
    (void)requireExistingProduct(productId); // validate product existence

    // Domain StockBatch requires a purchase date; we keep service in-memory
    // and use current time as "now".
...
bool WarehouseService::writeOffProduct(const std::string& productId,
    ...
    try {
        ...
        // The domain Warehouse signals "insufficient stock" with runtime_error.
        // The service converts that into graceful rejection (false).
        return false;
    }
}
...
void WarehouseService::deleteProduct(const std::string& productId) {
    ...
    // Remove all stock batches for this product from warehouse
